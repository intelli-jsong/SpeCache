...
from specache.utils_specache_quant import PerChannelQuantCache, PerTokenQuantCache
...
class ShadowKVCache:
    ...
    def __init__(self, 
        ...
        """
        {k/v}_cache_buffer position:
            [prefill_local | outlier_chunks | sparse_retrieved | generated_tokens]
                                            ↑                   ↑
                                        sparse_end         sparse_end+gen_offset

        v_cache_buffer (GPU):
            [prefill_local | outlier_chunks | sparse_retrieved | generated_tokens]
                ↑               ↑                  ↑                  ↑
            원본 그대로      원본 그대로     v_cache_cpu에서      원본 그대로
            (고정 저장)      (고정 저장)       동적 복사         (누적 저장)
        
        k_cache_buffer (GPU):
            [prefill_local | outlier_chunks | sparse_retrieved | generated_tokens]
                ↑               ↑                  ↑                  ↑
            RoPE 적용됨     RoPE 적용됨      U,SV에서 재구성     RoPE 적용됨
            (고정 저장)     (고정 저장)      +RoPE 적용        (그대로 저장)
        """
    ...
    def get_svd(self, new_k_cache, layer_idx):
        ...
        if new_k_cache.shape[1] <= 32:
            ...
            # [bsz, prefill_len, num_key_value_heads*head_dim]
            k_cache = new_k_cache
        ...
        """
        self.U[layer_idx].shape  : (bsz, seq_len, rank)
        self.SV[layer_idx].shape : (bsz, num_key_value_heads, rank, head_dim))
        """
    
    def register_k_landmark(self, k_landmark, k_landmark_idx, layer_idx):
        """
        k_landmark.shape:
            (bsz, num_key_value_heads, chunks - outlier_chunk, head_dim)
        k_landmark_idx.shape:
            (bsz, num_key_value_heads, chunks - outlier_chunk)
        """
    ...
    def prefill_kv_cache(self,
            ...
            query: torch.Tensor=None # what the heck is this query for?
            ):
        
        """
        new_v_cache.shape:
            (bsz, num_key_value_heads, seq_len, head_dim)
        key_states_roped.shape:
            (bsz, num_key_value_heads, seq_len, head_dim)
        """
        ...
        # local_chunk, rest에 대한 key, value state들은 k_cache_buffer, v_cache_buffer에 바로 저장
        ...
        landmark_candidates = key_states_roped_ctx.mean(dim=-2) # [bsz, num_key_value_heads, chunks, head_dim] -> 연속된 8개 key state들 마다 평균냄 
        """
        key_states_roped_ctx.shape:
            (bsz, num_key_value_heads, chunks, chunk_size, head_dim)
        landmark_candidates.shape:
            (bsz, num_key_value_heads, chunks, head_dim)
        """
        
        # compute the cos similarity between it and the original key cache
        cos_sim = torch.nn.functional.cosine_similarity(landmark_candidates.unsqueeze(3).expand(-1, -1, -1, self.chunk_size, -1), key_states_roped_ctx, dim=-1) # [bsz, num_key_value_heads, chunks, chunk_size]
        
        # get the outlier_chunk idx for each head 
        outlier_chunk_idx = cos_sim.min(dim=-1).values.topk(self.outlier_chunk, largest=False).indices
        """
        outlier_chunk_idx.shape:
            (bsz, num_key_value_heads, outlier_chunk=48)
            -> chunks에 대한 index들
        """
        ...
        """
        outlier_chunk_k_cache.shape:
            (bsz, num_key_value_heads, outlier_chunk*chunk_size, head_dim)
        outlier_chunk_v_cache.shape:
            (bsz, num_key_value_heads, outlier_chunk*chunk_size, head_dim)
        ...
        mask.scatter_(dim=-1, index=outlier_chunk_idx, value=False) # 각 num_key_value_heads 마다 outlier_chunk idx 위치를 False로 마킹
        ...
        self.register_k_landmark(landmark_candidates.gather(dim=2, index=rest_idx.unsqueeze(-1).expand(-1, -1, -1, self.head_dim)).view(self.batch_size, self.num_key_value_heads, -1, self.head_dim), 
                                 rest_idx, 
                                 layer_idx)
    ...
    def get_retrieval_position_ids(self, layer_idx, query_states):
        ...
        """
        query_states          : (1(b), num_key_value_heads(h), num_key_value_groups(g), 1(q), head_dim(d))
        k_landmark[layer_idx] : (1(b), num_key_value_heads(h), head_dim(d), chunks-outlier_chunk(c))
        chunk_attn            : (1(b), num_key_value_heads(h), num_key_value_groups(g), 1(q), chunks-outlier_chunk(c)) 
        """
        ...
        """
        chunk_attn.shape:
            (bsz, num_key_value_heads, num_key_value_groups, chunks-outlier_chunk)
        """
        if self.num_key_value_groups > 1:
            chunk_attn, _ = torch.max(chunk_attn, dim=-2) # [bsz, num_key_value_heads, chunks-outlier_chunk]
        merged_results = torch.topk(chunk_attn, k=self.select_sets, dim=-1).indices # [bsz, num_key_value_heads, select_sets(256)]

        # use merged_results to gather the position_ids: [bsz, 8, select_sets] --> [bsz, 8, select_sets]
        selected_chunks = self.k_landmark_idx[layer_idx].gather(dim=-1, index=merged_results) # [bsz, num_key_value_heads, select_sets(256)]
...
class SpeCacheKVCache:
    """SpeCache, only for accuracy measurement and understanding, not for efficiency"""
    def __init__(self, 
        config :object,
        batch_size :int = 1,
        max_length :int = 32*1024, 
        device :str = 'cuda:0',
        dtype = torch.bfloat16,
        quant_k_bit: int = 2, # 2 or 1 bit
        quant_v_bit: int = 2, # 2 or 1 bit
        quant_k_mode: str = 'channel', # 'channel' or 'token'
        quant_v_mode: str = 'token',  # 'channel' or 'token'
        do_specache_quant: bool = False,
        quant_group_size: int = 32, # 32 or 64
        quant_resuidual: int = 64, 
        arg_top_k: int = 64,
        cpu_mode: bool = True
        ) -> None:
        ...
        self.quant_k_bit = quant_k_bit
        self.quant_k_mode = quant_k_mode

        self.quant_v_bit = quant_v_bit
        self.quant_v_mode = quant_v_mode

        self.do_specache_quant = do_specache_quant

        self.cpu_mode = cpu_mode

        if self.quant_k_mode == 'token' or self.quant_v_mode == 'token':
            assert self.head_dim % quant_group_size == 0
        
        if self.quant_k_mode == 'channel' or self.quant_v_mode == 'channel':
            assert quant_resuidual % quant_group_size == 0

        self.quant_group_size = quant_group_size
        self.quant_residual = quant_resuidual
        self.arg_top_k = arg_top_k

        assert self.batch_size == 1, "SpeCacheKVCache class only supports batch_size=1 for now."

        """
        {k/v}_cache_cpu.shape:
            (num_layers, bsz=1, num_key_value_heads, max_length, head_dim)
        """
        
        # 16-bits full precision k cache (CPU Mem.)
        self.k_cache_cpu = torch.zeros(
            ...
            self.max_length,
            self.head_dim,
            device="cpu" if self.cpu_mode else self.device,
            dtype=self.dtype
        )
        
        # 16-bits full precision v cache (CPU Mem.)
        self.v_cache_cpu = torch.zeros(
            self.num_layers,
            batch_size,
            self.num_key_value_heads,
        ...
        if self.quant_k_mode == 'token':
            self.k_cache_gpu = PerTokenQuantCache(quant_group_size,
                                                    quant_k_bit,
                                                    do_specache_quant,
                                                    self.num_key_value_heads, 
                                                    batch_size, 
            ...
            self.k_cache_gpu = PerChannelQuantCache(quant_group_size,
                                                    quant_k_bit,
                                                    do_specache_quant,
                                                    self.num_key_value_heads, 
                                                    batch_size, 
        ...
        if self.quant_v_mode == 'token':
            self.v_cache_gpu = PerTokenQuantCache(quant_group_size,
                                                    quant_v_bit,
                                                    do_specache_quant,
                                                    self.num_key_value_heads, 
                                                    batch_size, 
            ...
            self.v_cache_gpu = PerChannelQuantCache(quant_group_size,
                                                    quant_v_bit,
                                                    do_specache_quant,
                                                    self.num_key_value_heads, 
                                                    batch_size, 
        ...
        self.retrieve_buffer_idx = torch.full(
            (self.num_layers, self.num_key_value_heads, self.arg_top_k),
            -1,  # -1 == invalid index
            device=self.device,
            dtype=torch.long
        )

        self.k_retrieve_buffer = torch.zeros(
            self.num_layers,
            batch_size,
            self.num_key_value_heads,
            self.arg_top_k,
        ...
        self.v_retrieve_buffer = torch.zeros(
            self.num_layers,
            batch_size,
            self.num_key_value_heads,
            self.arg_top_k,
    ...
    def print_stats(self):
        print("SpeCache : ")
        print(" > quant_k_bit:", self.quant_k_bit)
        print(" > quant_k_mode:", self.quant_k_mode)
        print(" > quant_v_bit:", self.quant_v_bit)
        print(" > quant_v_mode:", self.quant_v_mode)
        print(" > do_specache_quant:", self.do_specache_quant)
        print(" > quant_group_size:", self.quant_group_size)
        print(" > quant_residual:", self.quant_residual)
        print(" > arg_top_k:", self.arg_top_k)
        print(" > cpu_mode:", self.cpu_mode)
    ...
    def prefill_kv_cache(self,
        ...
        key_states_roped.shape:
            (bsz, num_key_value_heads, seq_len, head_dim)
        """
        incoming = value_states.shape[-2]

        # Algorithm 1 : K' = Quant(K), V' = Quant(V)
        self.k_cache_gpu.prefill(layer_idx, key_states_roped)
        self.v_cache_gpu.prefill(layer_idx, value_states)
            
        # Algorithm 1 : Offload(C)
        self.k_cache_cpu[layer_idx][:, :, :incoming] = key_states_roped.clone()
        self.v_cache_cpu[layer_idx][:, :, :incoming] = value_states.clone()

        if layer_idx == self.num_layers - 1:
            self.kv_offset = incoming
            self.prefill = incoming

    # Simplified version of concat low & high precision KV cache
    def get_mixed_precision_kv_beta(self, layer_idx):
        low_precision_k = self.k_cache_gpu.dequant_cache(layer_idx)
        low_precision_v = self.v_cache_gpu.dequant_cache(layer_idx)

        high_precision_idx = self.retrieve_buffer_idx[layer_idx].unsqueeze(0).to(torch.long)
        high_precision_k = self.k_retrieve_buffer[layer_idx]
        high_precision_v = self.v_retrieve_buffer[layer_idx]

        expended_idx = high_precision_idx.unsqueeze(-1).expand_as(high_precision_k)

        full_k = low_precision_k.scatter(dim=2, 
                                         index=expended_idx, 
                                         src=high_precision_k)
        full_v = low_precision_v.scatter(dim=2, 
                                         index=expended_idx, 
                                         src=high_precision_v)
    ...
    """
    (3.3. SPECACHE) evicting the non-top-k 16-bit KV pairs from VRAM that has been used, and offloading the newly created KV pairs
    """
    # Simplified version of KV cache prefetching
    def prefetch_kv_beta(self, layer_idx, retrieval_idx):
        """
        retrieval_idx.shape:
            (bsz, num_key_value_heads, 1, arg_top_k)
        """

        if self.cpu_mode:
            retrieval_idx_cpu = retrieval_idx.squeeze(2).unsqueeze(-1).expand(-1, -1, -1, self.head_dim).to("cpu")

            self.k_retrieve_buffer[layer_idx] = torch.gather(
                                                            self.k_cache_cpu[layer_idx],
                                                            dim=2,
                                                            index=retrieval_idx_cpu
                                                            ).to(self.device)
            
            self.v_retrieve_buffer[layer_idx] = torch.gather(
                                                            self.v_cache_cpu[layer_idx],
                                                            dim=2,
                                                            index=retrieval_idx_cpu
                                                            ).to(self.device)
            
        else:
            retrieval_idx_gpu = retrieval_idx.squeeze(2).unsqueeze(-1).expand(-1, -1, -1, self.head_dim)

            self.k_retrieve_buffer[layer_idx].copy_(torch.gather(
                                                            self.k_cache_cpu[layer_idx],
                                                            dim=2,
                                                            index=retrieval_idx_gpu
                                                            ))
            self.v_retrieve_buffer[layer_idx].copy_(torch.gather(
                                                            self.v_cache_cpu[layer_idx],
                                                            dim=2,
                                                            index=retrieval_idx_gpu
                                                            ))
            
        self.retrieve_buffer_idx[layer_idx].copy_(retrieval_idx.squeeze(2).squeeze(0))
    

    def update_kv_cache_gpu(self, 
            new_k_cache :torch.Tensor,
            new_v_cache :torch.Tensor,
            layer_idx :int,
            ):
        ...
        self.k_cache_gpu.update(layer_idx, new_k_cache)
        self.v_cache_gpu.update(layer_idx, new_v_cache)

        # self.kv_offset += incoming will be updated in update_kv_cache_cpu

    def update_kv_cache_cpu(self, 
            new_k_cache :torch.Tensor,
            new_v_cache :torch.Tensor,
            layer_idx :int,
            ):
    ...
    def clear(self):
        ...
        self.k_cache_gpu.clear()
        self.v_cache_gpu.clear()

        self.k_retrieve_buffer.zero_()
        self.v_retrieve_buffer.zero_()

        self.retrieve_buffer_idx.fill_(-1)
