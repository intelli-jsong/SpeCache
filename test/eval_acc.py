...
if __name__ == '__main__':
    ...
    if args.method == "specache":
        ...
        output_base_path = f"specache_archive/{model_name.split('/')[-1]}/{current_time}"
        os.makedirs(output_base_path, exist_ok=True)
        shutil.copy(f'{Path(__file__).parent.parent}/specache/config_quant_kv.yaml', output_base_path)
    ...
    for dataset_name in dataset_names:
        ...
        if args.method == "specache":
            output_path = f"{output_base_path}/{dataset_name}_{datalen}.jsonl"
        else:
