...
def get_pred(model, tokenizer, data, max_length, max_gen, prompt_format, dataset, device, model_name):
    ...
    if model.attn_mode.lower() == "specache":
        if model.check_hit_rate :
            hit_rate_path = f"{Path(__file__).parent.parent}/longbench_specache_hitrate/{model_name}_{model.specache_config_file.split('_')[1]}_{dataset}"
            os.makedirs(hit_rate_path, exist_ok=True)
    ...
    for json_obj in tqdm(data):
        ...
        if model.attn_mode.lower() == "specache":
            if model.check_hit_rate :
                sample_path = os.path.join(hit_rate_path, f"sample_{sample_num}.pt")
                sample_num += 1
            else:
                sample_path = None
...
def parse_args() -> Namespace:
    ...
    p.add_argument("--check_hit_rate", action='store_true', default=False)
...
if __name__ == '__main__':
    ...
    LLM = choose_model_class(model_full_name)

    max_len = model2maxlen[model_name]

    if args.e:
        datasets = ["qasper", "multifieldqa_en", "hotpotqa", "2wikimqa", "gov_report", "multi_news", 
                    "trec", "triviaqa", "samsum", "passage_count", "passage_retrieval_en", "lcc", "repobench-p"]
    elif args.check_hit_rate:
        datasets = ["gov_report", 
                    "multi_news"]
    else:
        # based on Results on https://openreview.net/forum?id=PQIrsaIQdn&noteId=mSvdThUKZd
        datasets = ["qasper",
                    "multifieldqa_en",
                    "hotpotqa",
                    "2wikimqa",
    ...
    for dataset in datasets:
        ...
        if args.e:
            ...
            # os.makedirs(f"longbench_pred_e/{model_name}_{max_len}_{args.k_bits}bits_group{args.group_size}_residual{model_args.residual_length}", exist_ok=True)
            # out_path = f"longbench_pred_e/{model_name}_{max_len}_{args.k_bits}bits_group{args.group_size}_residual{model_args.residual_length}/{dataset}.jsonl"
            raise NotImplementedError
        else:
            data = load_dataset('THUDM/LongBench', dataset, split='test')

            if args.method == 'specache':
                base_path = f"longbench_pred/{model_name}_{max_len}_{args.method}_{args.specache_bit}bit_seed{args.rand_seed}"
            else:
                base_path = f"longbench_pred/{model_name}_{max_len}_{args.method}_seed{args.rand_seed}"
            
            os.makedirs(base_path, exist_ok=True)
            
            if args.method == "specache":
                shutil.copy(f'{Path(__file__).parent.parent}/specache/specache_{args.specache_bit}bit_config.yaml', f"{base_path}/specache_config.yaml")

            out_path = f"{base_path}/{dataset}.jsonl"
        
        prompt_format = dataset2prompt[dataset]
        max_gen = dataset2maxlen[dataset]

        if args.method == 'specache':
            model = LLM(model_name=model_full_name, 
                        batch_size=batch_size, 
                        device=dist_config.device, 
                        max_length=max_len + max_gen,
                        ...
                        specache_config=f"specache_{args.specache_bit}bit_config.yaml",
                        check_hit_rate=args.check_hit_rate)
        else:
            model = LLM(model_name=model_full_name, 
                        batch_size=batch_size, 
                        device=dist_config.device, 
        ...
        preds = get_pred(model, model.tokenizer, data, max_len, max_gen, prompt_format, dataset, dist_config.device, model_name)

        del model

        if not args.check_hit_rate:
