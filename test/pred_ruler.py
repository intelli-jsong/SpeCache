...
def parse_args() -> Namespace:
    ...
    p.add_argument("--specache_bit", type=int, default=2)
...
if __name__ == '__main__':
    ...
    specache_bit = args.specache_bit
    ...
    if args.method == 'specache':
        llm = LLM(model_name=model_name, 
                    ...
                    specache_config=f"specache_{specache_bit}bit_config.yaml",
                    check_hit_rate=False)
    else:
        llm = LLM(model_name=model_name, 
                batch_size=batch_size, 
                device=dist_config.device, 
    ...
    for dataset_name in dataset_names:
        ...
        if args.method == "specache":
            output_path = f"ruler_pred/{model_name.split('/')[-1]}_spec_{specache_bit}bit/{dataset_name}_{datalen}.jsonl"
        elif args.method == "full":
            output_path = f"ruler_pred/{model_name.split('/')[-1]}_full/{dataset_name}_{datalen}.jsonl"
        else:
            output_path = f"ruler_pred/{model_name.split('/')[-1]}_shadowkv/{dataset_name}_{datalen}_{args.method}_{sparse_budget}_{rank}_{chunk_size}.jsonl"
